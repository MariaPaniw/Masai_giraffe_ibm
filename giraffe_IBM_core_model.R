  ...
  for(i in 1:(seasons-1)){ # second loop: SEASONS
    ...
    for(gg in 1:length(groups)){ # third loop: GROUP, which is social community
      ...
      if(nrow(DATA[DATA$age%in%c(1:3),])>0){
        ...
        DATA$surv[DATA$age%in%c(1:3)]=rbinom(length(DATA$surv[DATA$age%in%c(1:3)]),prob=survNeo(DATA$age[DATA$age%in%c(1:3)],DATA$age[DATA$age%in%c(1:3)]^2,nrow(DATA[DATA$age%in%c(1:87),]),DATA$beta3[DATA$age%in%c(1:3)],rain,com.effjuv.vec[gg], 
